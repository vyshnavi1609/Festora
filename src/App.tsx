...
const EventCard: React.FC<EventCardProps> = ({ event, user, onRegister, onUnregister, onSave, onMessage, onEdit, onRefresh, onViewProfile, onShare, onViewDetails, isRegistered, isLiked, onLike, onDelete }) => {
  ...
  const canEdit = event.created_by === user.id || user.role === 'admin' || user.role === 'council_president';
...
const Stories = ({ user, onViewProfile, onSendMessage }: { user: User, onViewProfile: (id: number) => void, onSendMessage?: (receiverId: number, content: string) => void }) => {
  ...
  const createStory = async () => {
    ...
    try {
      ...
      alert('Failed to post story. Check console for details.');
    }
  };
...
const SearchView = ({ events, user, onRegister, onUnregister, onSave, onMessage, onEdit, onRefresh, onViewProfile, onViewDetails, onBack, suggestions, onFollowSuggestion }: { events: Event[], user: User, onRegister: (id: number) => void, onUnregister: (id: number) => void, onSave: (id: number) => void, onMessage: (id: number) => void, onEdit: (event: Event) => void, onRefresh: () => void, onViewProfile: (userId: number) => void, onViewDetails: (event: Event) => void, onBack?: () => void, suggestions?: User[], onFollowSuggestion?: (id: number) => void }) => {
        ...
        <div className="flex flex-col gap-4 mb-10">
          {isOwnProfile ? (
            ...
            <>
              ...
              {targetUser?.role === 'club_president' && user.id !== targetUser?.id && (
                <>
